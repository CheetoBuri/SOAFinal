...
export function showAddToCart(productId, productName, price, category) {
    ...
    sizeModal.innerHTML = `
        <div class="modal-content" style="max-width: 500px;">
            ...
            <div class="modal-buttons">
                <button type="button" class="btn-submit" onclick="window.addToCartFromModal('${productId}', '${productName}', ${price}, ${isFood})">Add to Cart</button>
...
export function addToCartFromModal(productId, productName, price, isFood) {
    const size = document.getElementById('sizeSelect')?.value;
    if (!size) {
    ...
    if (!isFood) {
        ...
        sugar = document.getElementById('sugarSelect')?.value || '100';
    }
    
    addToCart(productId, productName, price, size, milks, sugar);
    closeSizeModal();
}

export function addToCart(productId, productName, basePrice, size, milks = [], sugar = '100') {
    if (!size) return;
    
    // Calculate final price with size modifier: ±10,000đ
    let finalPrice = basePrice;
        ...
        return `
            <div class="cart-item">
                ...
                <div class="cart-item-controls">
                    <button class="qty-btn" onclick='window.changeQty("${item.id}", "${item.size}", "${milksKey}", "${item.sugar || '100'}", -1)'>-</button>
                    <span style="padding:4px; flex:1; text-align:center;">${item.quantity}</span>
                    <button class="qty-btn" onclick='window.changeQty("${item.id}", "${item.size}", "${milksKey}", "${item.sugar || '100'}", 1)'>+</button>
                    <button class="remove-btn" onclick='window.removeFromCart("${item.id}", "${item.size}", "${milksKey}", "${item.sugar || '100'}")'>Remove</button>
                </div>
            </div>
        `;
    }).join('');
...
export function changeQty(productId, size, milksKey, sugar, change) {
    ...
    if (item) {
        ...
        if (item.quantity <= 0) {
            removeFromCart(productId, size, milksKey, sugar);
        } else {
            updateCartUI();
        }
    }
...
export function removeFromCart(productId, size, milksKey, sugar) {
